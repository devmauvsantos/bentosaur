...
def outside_window_signature(
    ...
    for vertex in body.data.vertices:
        x, y, z = vertex.co
        if abs(x) <= 0.130 and 0.380 <= z <= 0.550:
            continue
        coordinates.append(
            f"{x:.7f},{y:.7f},{z:.7f}"
        )
...
def create_skin_transition(
    ...
    inwards = (0.0000, 0.00035, 0.0009, 0.0017)
    loops = [
        loop_vertices(
...
def transfer_body_normals(
    obj: bpy.types.Object,
    body: bpy.types.Object,
) -> None:
    modifier = obj.modifiers.new(
        "R003_TRANSFER_BODY_NORMALS",
        "DATA_TRANSFER",
    )
...
def main() -> None:
    ...
    outer_window = scale_boundary(aperture, 1.10, 1.12)
    ...
    transfer_body_normals(skin_transition, body)
