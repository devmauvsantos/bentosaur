...
def append_scaffold(
    ...
    scaffold_candidates = [
        obj
        for obj in meshes
        if obj.get("bentosaur_pipeline_role")
        == "approved_bounded_retopology_scaffold"
        or obj.name == "SRC_H31_SMART_LOWPOLY_REPAIRED_LOCKED"
    ]
    if len(scaffold_candidates) == 1:
        scaffold = scaffold_candidates[0]
    elif len(meshes) == 1:
        scaffold = meshes[0]
    else:
        raise RuntimeError(
            "Expected one identifiable scaffold mesh in "
            f"{source}; found {len(meshes)} meshes and "
            f"{len(scaffold_candidates)} scaffold candidates"
        )
...
def enforce_body_origin_contract(
    body: bpy.types.Object,
) -> dict[str, object]:
    """Place the production work mesh exactly on X=0 and Z=0.

    The shared visual-source normalization can leave a retopology derivative
    a fraction of a millimetre off center or below the high-source floor
    because their extrema are not bit-identical. The production body owns the
    runtime origin contract, so correct only that editable derivative after
    the shared transform has been baked.
    """
    ...
    correction = Vector((-center_x, 0.0, -minimum_z))
    body.data.transform(Matrix.Translation(correction))
    body.data.update()
    return {
        "pre_correction_bounds": {
            "minimum_x": minimum_x,
            "maximum_x": maximum_x,
            "minimum_z": minimum_z,
        },
        "translation": list(correction),
        "post_condition": {
            "symmetry_plane": "local X = 0",
            "feet_floor": "minimum Z = 0",
        },
    }
...
def main() -> None:
    ...
    if args.stage == "S40":
        ...
        stage_payload["body_coordinate_correction"] = (
            enforce_body_origin_contract(body)
        )
