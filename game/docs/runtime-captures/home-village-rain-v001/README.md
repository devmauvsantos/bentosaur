# Home Village Rain V001 — Runtime Checkpoint

Captured from the isolated Godot lab scene on 2026-07-31.

- Engine: Godot 4.7 stable
- Renderer: Forward Mobile / Metal
- Canvas: 720 × 1280 portrait
- Review capture: 540 × 960 window override
- Fixed capture rate: 30 FPS
- Fixed particle seeds: enabled for reproducible review
- Scene: `res://scenes/labs/home_village_rain_lab.tscn`
- State: full rain density, registered lights awake

Files:

- `home-village-rain-v001-checkpoint.png`: representative settled frame
- `home-village-rain-v001-motion.gif`: compact review loop
- `home-village-rain-v001-motion.mp4`: full-quality motion checkpoint

This is a visual checkpoint, not final approval. No stall, dinosaurs, UI, or
gameplay are present.
